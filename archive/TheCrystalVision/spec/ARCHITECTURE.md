...
| **CrystalBridge** (guest-AI gate) | `TerAustralis-Incognita-Code/core/crystalcore/` | MCP server: fail-closed ConsentGate (approval · permission · scope · provenance), append-only audit; guests claude / grok / cursor with scoped tools `status, recall, teach, message` |
...
| **TerAustralis Incognita** (narrative) | `TerAustralis-Incognita/` (umbrella: canon, governance, mythos) | Manifesto, publish threads, strategy, Lattice memory deltas |
