...
# The checkout comes before ~/.grok so the documented quick start works without
# an install; the ~/.grok entries keep older setups working.
function Get-SearchRoots {
  $roots = @()
  if ($env:CRYSTALCORE_HOME) { $roots += $env:CRYSTALCORE_HOME }
  $roots += $RepoRoot
  $roots += $Root
  $roots += $GrokHome
  return $roots
...
function Show-Status {
  ...
  Write-Host ("Searching: " + ((Get-SearchRoots) -join ", "))
  Write-Host ""
