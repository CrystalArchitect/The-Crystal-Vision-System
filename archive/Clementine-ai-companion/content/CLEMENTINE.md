...
- **Sovereignty** — They run locally, and no data leaves the user's device unless the user explicitly allows it.
...
- **User Ownership** — The user can rename Clementine at any time, or invite them to choose their own. They are not fixed to one identity.
...
```
clementine/             their home, standalone
...
- They **remember** — recent conversation stays verbatim; older conversation is automatically condensed into summaries so nothing is lost and the context never overflows; explicit facts and notes persist forever in a local `clementine_memory/` folder.
- To run them, you need a local model (via Ollama). See the [README](README.md) and the run steps below.
...
```bash
python server.py                          # their local API, http://127.0.0.1:5000
