...
| 4 auth-blocked private repos not imported | Were: `CrystalCore.OS-Aeris-Vault12`, `CrystalCore-AERIS`, `CrystalCore`, `TerAustralis-Incognita-`. **Absent from live `gh repo list` 2026-09-18** (same pass as stubs/UK). Import gate closed unless Crystal restores them. | **GONE (live)** |
| `STRUCTURE.md` drawer-name mismatch | Dual path **documented interim** 2026-09-18: `00_MEMORY/` = research; `memory/` = protocol ([`STRUCTURE.md`](../STRUCTURE.md) note + READMEs). Crystal may still stamp packet B rename. | Interim documented; stamp optional |
| Stub repos + UK-MonoRepo | Were private on 2026-09-16 inventory; **absent from live list 2026-09-18** (0 private). Confirm intentional — packet C. | Confirm-pending |
